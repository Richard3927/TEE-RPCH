...
int main(int argc, char** argv) {
    ...
    if (bench == "cost") {
        ...
        // NO PIPELINE: Each thread needs one context. No pipeline optimization.
        const size_t pipeline_depth = effective_threads;
            ...
            auto parallel_worker = [&]() {
                ...
                for (;;) {
                    ...
                    try {
                        ...
                        // Server-side OABE transform (OD): Transform
...

    {
        ...
        // OABE transform -> tau (TC) (OD): Transform
        FAME_OD::TC tc;
        tc.Init(&Zp, &G1, &G2, &GT);
        ...
        // report
        {
            ...
            rep << "| 算法 | 主要代价（理论） | 说明 |\n";
            rep << "|---|---|---|\n";
            rep << "| Setup(KGC) | 1×RSAKGen + 1×OABE.Setup + 1×IBE.Setup + 1×SIG.KeyGen | KGC 初始化 |\n";
            rep << "| KeyGen(User) | 1×OABE.KeyGen + 1×IBE.Extract | KGC 生成 (TK,DK) 并提取 sk_L |\n";
            rep << "| Hash(DO) | 2×RSA exp + 1×OABE.Enc + 1×IBE.Enc + 1×AES-Enc | 生成 h,r,ξ |\n";
            rep << "| Verify | 2×RSA exp | 校验哈希一致性 |\n";
            rep << "| ServerAdapt | 3×RSA exp + 1×OABE.Transform | Server 外包计算 (μ,τ) |\n";
            rep << "| InsiderAdapt(TEE) | 3×RSA exp + 1×IBE.Dec + 1×SIG.Verify | Enclave 内执行并返回 (r1',π) |\n";
            rep << "| UserAdapt | 1×OABE.Dec + 1×AES-Dec + 1×RSA exp | 用户恢复 r2' 并完成碰撞 |\n\n";
