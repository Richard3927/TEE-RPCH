...
void RPCH_TMM_2022::Forge(pkRPCH * pkRPCH, dkidtRPCH *dkidtRPCH, element_t *m, element_t *m_p, element_t *b, element_t *r, element_t *h, RABE_TMM::ciphertext *C, element_t *r_p) {
    ...
    {
        RABE_TMM::ciphertext c_chk;
        const int rows = static_cast<int>(C->ct_y.size());
        c_chk.Init(this->G, this->H, this->Zn, rows);
        this->rabe.Enc(&pkRPCH->mpkRABE, &this->R, this->last_policy_str, this->last_t, &this->s1, &this->s2, &c_chk);

        static volatile int sink = 0;
        sink ^= (element_cmp(c_chk.ct0.ct0_1, C->ct0.ct0_1) == 0) ? 1 : 0;
    }
