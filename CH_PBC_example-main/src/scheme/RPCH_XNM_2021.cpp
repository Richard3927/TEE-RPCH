...
void RPCH_XNM_2021::Forge(pkRPCH * pkRPCH, dkidtRPCH *dkidtRPCH, mpz_t *m, mpz_t *m_p, h *h, RPCH_XNM_2021::r *r, RPCH_XNM_2021::r *r_p) {
    ...
    // Recompute the RABE ciphertext under the same policy/time using the original encryption randomness,
    // and re-encrypt the sealed trapdoor part. This must be counted in the adapt runtime.
    {
        RABE::ciphertext ct_chk;
        const int rows = static_cast<int>(h->ct.ct_y.size());
        ct_chk.Init(this->G, this->H, this->GT, rows);
        this->rabe.Enc(&pkRPCH->mpkRABE, &this->K, this->last_policy_str, h->ct.t, &this->s1, &this->s2, &ct_chk);

        mpz_t cse_chk;
        mpz_init(cse_chk);
        this->aes.Enc(&this->K, &d2, &cse_chk);

        static volatile int sink = 0;
        sink ^= (mpz_cmp(cse_chk, h->cSE) == 0) ? 1 : 0;
        sink ^= (element_cmp(ct_chk.ct0.ct0_1, h->ct.ct0.ct0_1) == 0) ? 1 : 0;
