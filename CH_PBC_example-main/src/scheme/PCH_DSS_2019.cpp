...
void PCH_DSS_2019::Hash(pkPCH *pkPCH, mpz_t *m, string policy_str, h *h,r *r) {
    ...
    this->last_policy_str = policy_str;
    ...
    // (u1,u2) <- H4((r,A)); use the actual randomness (derived from hash randomness r1/r2).
    // The previous version used an uninitialized value here, which effectively removed this binding.
    H4(&r->r1, policy_str, &u1, &u2);
...
void PCH_DSS_2019::Forge(pkPCH * pkPCH, sksPCH *sksPCH, mpz_t *m, mpz_t *m_p, h *h, PCH_DSS_2019::r *r, PCH_DSS_2019::r *r_p) {
    ...
    // Re-encryption / consistency-check cost (omitted previously):
    // Recompute the ABE ciphertext under the same policy using the original hash randomness,
    // and re-encrypt the sealed trapdoor part. This matches the "re-encryption" step in the
    // original Adapt procedure and must be counted in the adapt runtime.
    {
        element_t u1_chk, u2_chk;
        element_init_same_as(u1_chk, *this->Zn);
        element_init_same_as(u2_chk, *this->Zn);
        H4(&r->r1, this->last_policy_str, &u1_chk, &u2_chk);

        CP_ABE::ciphertext ct_chk;
        const int rows = static_cast<int>(h->ct.ct_y.size());
        ct_chk.Init(this->G, this->H, this->GT, rows);
        this->cp_abe.Encrypt(&pkPCH->mpkABE, &this->K, this->last_policy_str, &u1_chk, &u2_chk, &ct_chk);

        mpz_t ct_se_chk;
        mpz_init(ct_se_chk);
        this->aes.Enc(&this->K, &d2, &ct_se_chk);

        static volatile int sink = 0;
        sink ^= (mpz_cmp(ct_se_chk, h->ct_) == 0) ? 1 : 0;
        sink ^= (element_cmp(ct_chk.ct0.ct_1, h->ct.ct0.ct_1) == 0) ? 1 : 0;

        mpz_clear(ct_se_chk);
        element_clear(u1_chk);
        element_clear(u2_chk);
    }
